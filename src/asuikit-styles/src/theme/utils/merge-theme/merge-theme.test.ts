...
describe('@asuikit/styles/merge-theme', () => {
  ...
  it('get font color', () => {
    const themeBase = getThemeBase();
    const lightTheme = attachFunctions(mergeTheme(themeBase, { colorScheme: 'light' }));

    expect(
      lightTheme.fn.variant({ variant: 'filled', color: 'currentColor', primaryFallback: false })
        .color
    ).toEqual('#0A0A0A');

    const darkTheme = attachFunctions(mergeTheme(themeBase, { colorScheme: 'dark' }));
