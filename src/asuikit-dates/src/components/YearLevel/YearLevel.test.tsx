...
describe('@asuikit/dates/YearLevel', () => {
  itSupportsSystemProps({
    component: YearLevel,
    props: defaultProps,
    refType: HTMLDivElement,
    providerName: 'YearLevel',
    displayName: '@asuikit/dates/YearLevel',
  });
  ...
  it('has correct default __staticSelector', () => {
    ...
    expect(container.firstChild).toHaveClass('asuikit-YearLevel-yearLevel');
    expect(container.querySelector('table td button')).toHaveClass(
      'asuikit-YearLevel-pickerControl'
    );
    expect(screen.getByLabelText('level-control')).toHaveClass(
      'asuikit-YearLevel-calendarHeaderLevel'
    );
  });

  it('supports custom __staticSelector', () => {
    const { container } = render(<YearLevel {...defaultProps} __staticSelector="Calendar" />);
    expect(container.firstChild).toHaveClass('asuikit-Calendar-yearLevel');
    expect(container.querySelector('table td button')).toHaveClass(
