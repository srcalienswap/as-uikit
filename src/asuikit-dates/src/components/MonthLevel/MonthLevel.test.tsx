...
describe('@asuikit/dates/MonthLevel', () => {
  itSupportsSystemProps({
    component: MonthLevel,
    props: defaultProps,
    refType: HTMLDivElement,
    providerName: 'MonthLevel',
    displayName: '@asuikit/dates/MonthLevel',
  });
  ...
  it('has correct default __staticSelector', () => {
    ...
    expect(container.firstChild).toHaveClass('asuikit-MonthLevel-monthLevel');
    expect(container.querySelector('table td button')).toHaveClass('asuikit-MonthLevel-day');
    expect(screen.getByLabelText('level-control')).toHaveClass(
      'asuikit-MonthLevel-calendarHeaderLevel'
    );
  });

  it('supports custom __staticSelector', () => {
    const { container } = render(<MonthLevel {...defaultProps} __staticSelector="Calendar" />);
    expect(container.firstChild).toHaveClass('asuikit-Calendar-monthLevel');
    expect(container.querySelector('table td button')).toHaveClass('asuikit-Calendar-day');
    expect(screen.getByLabelText('level-control')).toHaveClass(
      'asuikit-Calendar-calendarHeaderLevel'
    );
  });
