import { createStyles, MantineNumberSize, MantineTheme, rem } from '@asuikit/core';
...
export default createStyles(
  (theme: MantineTheme, { radius, background }: ConfiguratorStylesParams) => ({
    configurator: {
      display: 'flex',
      maxWidth: '100%',
      border: `${rem(1)} solid ${theme.colors.line[2]}`,
      overflow: 'hidden',
    ...
    noCode: {
      ...
      borderBottom: `${rem(1)} solid ${theme.colors.line[2]}`,
    },
    ...
    controls: {
      ...
      borderLeft: `${rem(1)} solid ${theme.colors.line[2]}`,

      [`@media (max-width: ${BREAKPOINT})`]: {
        width: '100%',
        borderLeft: 0,
        borderTop: `${rem(1)} solid ${theme.colors.line[2]}`,
      },
    },
