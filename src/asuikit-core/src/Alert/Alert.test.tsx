...
describe('@asuikit/core/Alert', () => {
  ...
  itSupportsSystemProps({
    component: Alert,
    props: defaultProps,
    displayName: '@asuikit/core/Alert',
  ...
  it('renders close button based on withCloseButton prop', () => {
    ...
    expect(withCloseButton.querySelector('.asuikit-Alert-closeButton')).toBeInTheDocument();
    expect(withoutCloseButton.querySelectorAll('.asuikit-Alert-closeButton')).toHaveLength(0);
  });
  ...
  it('does not render title if title prop was not passed', () => {
    ...
    expect(container.querySelectorAll('.asuikit-Alert-title')).toHaveLength(0);
  });
