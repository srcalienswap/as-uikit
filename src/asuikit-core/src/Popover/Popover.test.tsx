...
import { itRendersChildren, checkAccessibility, wait, renderWithAct } from '@asuikit/tests';
...
describe('@asuikit/core/Popover', () => {
  ...
  it('correctly handles withArrow={true}', () => {
    ...
    expect(container.querySelectorAll('.asuikit-Popover-arrow')).toHaveLength(1);
  });

  it('correctly handles withArrow={false}', () => {
    const { container } = render(<TestContainer defaultOpened withArrow={false} />);
    expect(container.querySelectorAll('.asuikit-Popover-arrow')).toHaveLength(0);
  });
