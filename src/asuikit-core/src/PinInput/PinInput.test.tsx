...
describe('@asuikit/core/PinInput', () => {
  checkAccessibility([<PinInput {...defaultProps} aria-label="Test label" />]);
  itSupportsSystemProps({
    component: PinInput,
    props: defaultProps,
    displayName: '@asuikit/core/PinInput',
  ...
  it('renders correct amount of inputs based on length prop', () => {
    ...
    expect(container.querySelectorAll('.asuikit-PinInput-input')).toHaveLength(5);
  });
  ...
  it('onComplete is called on last input', () => {
    ...
    expect(container.querySelectorAll('.asuikit-PinInput-input')).toHaveLength(4);

    container.querySelectorAll('.asuikit-PinInput-input').forEach((element) => {
  ...
  it('stay focused on last element on initial backspace press', async () => {
    ...
    container.querySelectorAll('.asuikit-PinInput-input').forEach((element, key) => {
      fireEvent.change(element, { target: { value: key } });
    });

    await userEvent.keyboard('{Backspace}');
    expect(container.querySelectorAll('.asuikit-PinInput-input')[4]).toHaveFocus();
  });
  ...
  it('inputs except the last element should not need two backspace press to shift focus to prev element', async () => {
    ...
    await userEvent.keyboard('{Backspace}');
    expect(container.querySelectorAll('.asuikit-PinInput-input')[3]).toHaveFocus();

    await userEvent.keyboard('{Backspace}');
    expect(container.querySelectorAll('.asuikit-PinInput-input')[2]).toHaveFocus();

    await userEvent.keyboard('{Backspace}');
    expect(container.querySelectorAll('.asuikit-PinInput-input')[1]).toHaveFocus();

    await userEvent.keyboard('{Backspace}');
    expect(container.querySelectorAll('.asuikit-PinInput-input')[0]).toHaveFocus();
  });
  ...
  it('inputs will be filled when the value is bigger than 2 chars(ex: Gboard paste action from keypad)', async () => {
    ...
    const element = container.querySelectorAll('.asuikit-PinInput-input')[0];
