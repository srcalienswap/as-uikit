...
export function itSupportsInputProps<P>(
  ...
  it('handles error and invalid state', () => {
    ...
    expect(invalid.querySelectorAll(`.asuikit-${name}-error`)).toHaveLength(0);
    expect(withError.querySelector(`.asuikit-${name}-error`)).toBeInTheDocument();
    expect(withError.querySelector(`.asuikit-${name}-error`).textContent).toBe('Test error');
  });
