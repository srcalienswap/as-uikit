...
You can create these styles on your own or add them by setting `withGlobalStyles` prop on [MantineProvider](/theming/asuikit-provider/), which includes them by default:
...
import { MantineProvider, ColorSchemeProvider, ColorScheme } from '@asuikit/core';
...
import { MantineProvider, ColorScheme, ColorSchemeProvider } from '@asuikit/core';
...
export default function App(props: AppProps & { colorScheme: ColorScheme }) {
  ...
  const toggleColorScheme = (value?: ColorScheme) => {
    ...
    setCookie('asuikit-color-scheme', nextColorScheme, { maxAge: 60 * 60 * 24 * 30 });
  };
...
App.getInitialProps = async (appContext: AppContext) => {
  ...
  return {
    ...appProps,
    colorScheme: getCookie('asuikit-color-scheme', appContext.ctx) || 'light',
  };
};
```
