...
```bash
npx create-remix@latest my-asuikit-app
```

<PackagesInstallation extraPackages={['@asuikit/remix', '@emotion/server']} />
...
import { injectStyles, createStylesServer } from '@asuikit/remix';
...
Add [MantineProvider](/theming/asuikit-provider/) and `StylesPlaceholder` to the `root.tsx` file:

```tsx
import type { MetaFunction } from '@remix-run/node';
import { Links, LiveReload, Meta, Outlet, Scripts, ScrollRestoration } from '@remix-run/react';
import { MantineProvider, createEmotionCache } from '@asuikit/core';
import { StylesPlaceholder } from '@asuikit/remix';
...
import { ClientProvider } from '@asuikit/remix';
