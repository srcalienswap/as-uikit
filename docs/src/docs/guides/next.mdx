...
<PackagesInstallation extraPackages={['@asuikit/next', '@emotion/server']} />

Create `pages/_document.tsx` file:

```tsx
import { createGetInitialProps } from '@asuikit/next';
...
Add [MantineProvider](/theming/asuikit-provider/) to `pages/_app.tsx`:
