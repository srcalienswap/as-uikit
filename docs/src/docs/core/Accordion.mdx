...
import: "import { Accordion } from '@asuikit/core';"
source: 'asuikit-core/src/Accordion/Accordion.tsx'
docs: 'core/Accordion.mdx'
styles: ['Accordion']
---

import { AccordionDemos } from '@asuikit/demos';
...
```tsx
import { Accordion, rem } from '@asuikit/core';
...
`AccordionProps` type exported from `@asuikit/core` is a generic, it accepts boolean type that
describes `multiple` state:

```tsx
import type { AccordionProps } from '@asuikit/core';
