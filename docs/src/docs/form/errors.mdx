...
`form.errors` type is `Record<string, React.ReactNode>`, you can import a shorthand `FormErrors` type from `@asuikit/form`:

```tsx
import type { FormErrors } from '@asuikit/form';
```
