...
import: "import { Dropzone } from '@asuikit/dropzone';"
source: 'asuikit-dropzone/src'
docs: 'others/dropzone.mdx'
installation: '@asuikit/dropzone'
license: MIT
...
} from '@asuikit/dropzone';
import { DropzoneDemos } from '@asuikit/demos';
...
```bash
yarn add @asuikit/dropzone
```

Install with npm:

```bash
npm install @asuikit/dropzone
```
...
To save some research time you can use `MIME_TYPES` variable exported from `@asuikit/dropzone`:

```tsx
import { Dropzone, MIME_TYPES } from '@asuikit/dropzone';
...
```tsx
import { IMAGE_MIME_TYPE, Dropzone } from '@asuikit/dropzone';
