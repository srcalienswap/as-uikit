...
import: "import { RichTextEditor } from '@asuikit/tiptap';"
source: 'asuikit-tiptap/src'
docs: 'others/tiptap.mdx'
installation: '@asuikit/tiptap'
license: MIT
...
```bash
yarn add @asuikit/tiptap @asuikit/core @asuikit/hooks @tabler/icons-react @tiptap/react @tiptap/pm @tiptap/extension-link @tiptap/starter-kit
```

Install with npm:

```bash
npm install @asuikit/tiptap @asuikit/core @asuikit/hooks @tabler/icons-react @tiptap/react @tiptap/pm @tiptap/extension-link @tiptap/starter-kit
```

## TipTap editor

`@asuikit/tiptap` provides a UI for [Tiptap](https://tiptap.dev/). `RichTextEditor` component
...
`@asuikit/tiptap` provides custom `Link` extension that is required to be used instead of
`@tiptap/extension-link` in order for `Ctrl + K` keyboard shortcut to work:

```tsx
// Use Link extension exported from the @asuikit/tiptap package
...
```tsx
import { useRichTextEditorContext } from '@asuikit/tiptap';
...
```tsx
// RichTextEditorLabels type can be imported from @asuikit/tiptap package
...
Default labels (can be imported from `@asuikit/tiptap` package):
