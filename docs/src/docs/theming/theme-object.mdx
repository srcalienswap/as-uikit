...
order: 0
search: 'Theme object, use-asuikit-theme, theme overrides, MantineTheme type'
---
...
To customize theme, pass theme override object to [MantineProvider](/theming/asuikit-provider/) `theme` prop.
Theme override will be deeply merged with [default theme](https://github.com/mantinedev/mantine/blob/master/src/asuikit-styles/src/theme/default-theme.ts).
...
```tsx
import { MantineThemeOverride, MantineProvider } from '@asuikit/core';
...
`use-asuikit-theme` hook returns theme from [MantineProvider](/theming/asuikit-provider/) context or default theme
...
You can import `MantineTheme` type from `@asuikit/core` package:

```tsx
import { useMantineTheme, MantineTheme } from '@asuikit/core';
